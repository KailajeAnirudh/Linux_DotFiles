(function() {
    ...
    queryById("unlockButton").onclick = function() {
        ...
        if (enteredPassword.length != 0) {
            browser.storage.local.get(getConst.passwordLockingPasswordData, function (obj) {
                const data = obj[getConst.passwordLockingPasswordData];
                
                if (data != null) {
                    if ((data == enteredPassword) || (enteredPassword == generateUniqueCode())) {
    ...
    queryById("passwordUnlockingShowPromptButton").onclick = function() {
        browser.storage.local.get(passwordLockingPromptData, function (obj) {
            const prompt = obj[passwordLockingPromptData] ?? "";
            
            if (prompt != "") {
                queryById("passwordUnlockingPromptDisplaying").innerHTML = prompt;
                queryById("passwordUnlockingPromptDisplaying").style.display = "block";
                queryById("passwordUnlockingShowPromptButton").classList.add("invisible");
            }
        })
    }
