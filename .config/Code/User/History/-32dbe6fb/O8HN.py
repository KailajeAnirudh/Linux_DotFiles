...
def graph_search(world, resolution, margin, start, goal, astar):
    ...
    print(f"Original resolution: {resolution}, Map Shape: {np.prod(original_map.map.shape)}")
...
@dataclass
class Node():
    def __init__(self, index, g=0, f=0, parent=None, close_stat = False):
        self.index = index
        self.g = g
        self.f = f
        self.parent = parent
...
class GraphSearchPathPlanner():
    def __init__(self, start_idx, goal_idx, occ_map, astar):
        ...
        heappush(self.priority_queue, Node(self.start_idx, 0, 0, None, False))
    ...
    def search(self):
        ...
        while self.open_set and not self.goal_idx in self.closed_set.keys():
            ...
            for neighbour in valid_neighbours:
                total_cost = self.current_node.g + ((neighbour[0]-self.current_node.index[0])**2 
                                                    + (neighbour[1]-self.current_node.index[1])**2 
                                                    + (neighbour[2]-self.current_node.index[2])**2)**0.5
                self.linalg_calls += 1

                if tuple(neighbour) not in self.open_set and tuple(neighbour) not in self.closed_set:
                    f = total_cost
                    if self.astar:
                        f += ((neighbour[0]-self.goal_idx[0])**2 
                            + (neighbour[1]-self.goal_idx[1])**2 
                            + (neighbour[2]-self.goal_idx[2])**2)**0.5
                    neighbour_node = Node(tuple(neighbour), g = total_cost, f= f, parent=self.current_node.index, close_stat=False)
                    ...
                    self.open_dict_index += 1
                elif tuple(neighbour) in self.open_set and total_cost < self.open_set[tuple(neighbour)].g:
