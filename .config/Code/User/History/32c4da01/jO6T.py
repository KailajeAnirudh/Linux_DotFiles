...
class Quadrotor(object):
  ...
  def add_initial_state_constraint(self, prog, x, x_current):
    ...
    prog.AddLinearEqualityConstraint(x[0], x_current)
  ...
  def add_input_saturation_constraint(self, prog, x, u, N):
    ...
    for i in range(len(u)):
      prog.AddBoundingBoxConstraint(self.umin-self.m*self.g/2, self.umax-self.m*self.g/2, u[i])
  ...
  def add_dynamics_constraint(self, prog, x, u, N, T):
    ...
    for i in range(1, 10):
      prog.AddLinearEqualityConstraint(x[i] - (A @ x[i-1] + B@u[i-1]), np.zeros_like(x[i]))

  def add_cost(self, prog, x, u, N):
    # TODO: add cost.
    # print(u.shape, x.shape, N)
    for i in range(N-1):
      prog.AddQuadraticCost(0.5*(x[i+1].T@self.Q@x[i+1] + u[i].T@self.R@u[i]))
  ...
  def compute_mpc_feedback(self, x_current, use_clf=False):
    ...
    # printed = False
    # if not printed:
    #     print(result.GetSolution(u).shape)
    #     printed = True
