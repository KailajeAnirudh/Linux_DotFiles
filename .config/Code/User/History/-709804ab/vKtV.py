...
class SE3Control(object):
    ...
    def __init__(self, quad_params):
        ...
        # STUDENT CODE HERE
        self.Kp = np.diag([8, 8, 19.0])*1.15#/4.2
        self.Kd = np.diag([5.5,5.5,9.0])*1.05#/2.5
        ...
        self.motor_speed_calc_matrix = np.linalg.inv(np.array([[1,1,1,1],
    ...
    def update(self, t, state, flat_output):
        ...
        # cmd_motor_speeds = np.clip(u, 0, 1e7)

        cmd_motor_speeds = np.sign(u)*np.sqrt(np.abs(u)/self.k_thrust)
