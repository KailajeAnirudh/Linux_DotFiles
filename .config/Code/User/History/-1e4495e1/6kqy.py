...
class OSC(LeafSystem):
    ...
    def solveQP(self, context):
        ...
        if self.fsm != FLIGHT and t > 0.1:
            ...
            # qp.AddLinearConstraint(np.ones_like(statePacket['left_leg'], dtype=np.float64), np.zeros_like(statePacket['left_leg'], dtype=np.float64), np.inf*np.ones_like(statePacket['left_leg'], dtype=np.float64), statePacket['left_leg']) #Eq 8 
            qp.AddLinearConstraint(statePacket['left_leg'][-1] >= np.zeros_like(statePacket['left_leg'][-1], dtype=np.float64)-1e-2) 
            ...
            qp.AddLinearConstraint(friction_force(lambda_c[4:]), np.zeros_like(friction_force(lambda_c[:4])), np.inf*np.ones_like(friction_force(lambda_c[:4])))
            """Eq 11, Pg 8 from https://groups.csail.mit.edu/robotics-center/public_papers/Posa13.pdf"""
            ...
            """Eq 12, Pg 8 from https://groups.csail.mit.edu/robotics-center/public_papers/Posa13.pdf"""
            qp.AddLinearConstraint(gamma[:3]+statePacket['leftVel'], np.zeros_like(gamma[:3]), np.inf*np.ones_like(gamma[:3]))
            qp.AddLinearConstraint(gamma[3:]+statePacket['rightVel'],np.zeros_like(gamma[:3]), np.inf*np.ones_like(gamma[:3]))
            qp.AddLinearConstraint(gamma[:3]-statePacket['leftVel'], np.zeros_like(gamma[:3]), np.inf*np.ones_like(gamma[:3]))
            qp.AddLinearConstraint(gamma[3:]-statePacket['rightVel'],np.zeros_like(gamma[:3]), np.inf*np.ones_like(gamma[:3]))
            """Eq 13, Pg 8 from https://groups.csail.mit.edu/robotics-center/public_papers/Posa13.pdf"""
            qp.AddLinearEqualityConstraint(statePacket['left_leg']@lambda_c[4] == np.zeros_like(statePacket['left_leg']@lambda_c[4]))
            qp.AddLinearEqualityConstraint(statePacket['right_leg']@lambda_c[7] == np.zeros_like(statePacket['left_leg']@lambda_c[4]))
            """Eq 14, Pg 8 from https://groups.csail.mit.edu/robotics-center/public_papers/Posa13.pdf"""
            qp.AddLinearEqualityConstraint((gamma[:3]+statePacket['leftVel']) @ lambda_c[0], np.zeros_like((gamma[:3]+statePacket['leftVel']) @ lambda_c[0]))
            qp.AddLinearEqualityConstraint((gamma[:3]+statePacket['rightVel']) @ lambda_c[4], np.zeros_like((gamma[:3]+statePacket['leftVel']) @ lambda_c[0]))
            """Eq 15, Pg 8 from https://groups.csail.mit.edu/robotics-center/public_papers/Posa13.pdf"""
            qp.AddLinearEqualityConstraint((gamma[:3]-statePacket['leftVel']) @ lambda_c[1], np.zeros_like((gamma[:3]+statePacket['leftVel']) @ lambda_c[0]))
            qp.AddLinearEqualityConstraint((gamma[:3]-statePacket['rightVel']) @ lambda_c[5], np.zeros_like((gamma[:3]+statePacket['leftVel']) @ lambda_c[0]))
            """y- contact forces at 0"""
            qp.AddLinearEqualityConstraint(lambda_c[1] == np.zeros_like(lambda_c[1])) #contact y
            qp.AddLinearEqualityConstraint(lambda_c[4] == np.zeros_like(lambda_c[1]))
