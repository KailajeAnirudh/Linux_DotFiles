...
class OSC(LeafSystem):
    ...
    def solveQP(self, context):
        ...
        if self.fsm != FLIGHT and t > 0.1:
            ...
            qp.AddConstraint(Eq13Helper, np.zeros_like(statePacket['left_leg']@lambda_c[4], dtype=np.float64), np.zeros_like(statePacket['left_leg']@lambda_c[4], dtype=np.float64), vars = np.hstack([]))
