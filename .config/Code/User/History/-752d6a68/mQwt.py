...
def find_throwing_trajectory(N, initial_state, jumpheight, tf, jumpheight_tol=5e-2):
  ...
  """Joint limits specified in the order [planar_x: m, planar_y:m, roty(torso_angle): radians, left_hip:radians, right_hip: radians, left_knee: radians, right_knee: radians]"""
  ...
  x = np.zeros((N, n_x), dtype="object"); u = np.zeros((N, n_u), dtype="object")
  for i in range(N):
  ...
  velocity_tol = (2*g*jumpheight_tol)**0.5
  # prog.AddLinearEqualityConstraint(xf[n_q+1], final_configuration)
  prog.AddBoundingBoxConstraint(-velocity_tol, velocity_tol, xf[n_q+1]-required_velocity)
  ...
  AddCollocationConstraints(prog, robot, context, N, x, u, timesteps)
  ...
  x_init = np.zeros((N, n_x))
  # x_init[:, 0] = np.linspace(0,np.pi, N)
...
if __name__ == '__main__':
  ...
  tf = 3.0
  distance = 15.0
  x_traj, u_traj, prog, _, _ = find_throwing_trajectory(N, initial_state, jumpheight = 1.5, tf, jumpheight_tol=5e-2)