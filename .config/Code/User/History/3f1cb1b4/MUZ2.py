...
class GraphSearchPathPlanner():
    ...
    def search(self):
        Result_Node = None
        Expanded_Nodes = None
        while self.priority_queue and not self.goal_idx in self.closed_set.keys():
            ...
            if self.current_node == self.goal_idx:
                ...
                while self.current_node != self.start_idx:
                    self.Path.append(self.occ_map.index_to_metric_center(np.array(self.current_node)))
                    self.current_node = self.closed_set[self.current_node].parent
            ...
            for neighbour in valid_neighbours:
                ...
                total_cost = self.costs[self.current_node] + np.linalg.norm(np.array(neighbour) - np.array(self.current_node.index))
                self.linalg_calls += 1
                if self.astar:
