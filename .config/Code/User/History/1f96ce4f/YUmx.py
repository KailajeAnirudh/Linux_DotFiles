...
def CollocationConstraintEvaluator(planar_arm, context, dt, x_i, u_i, x_ip1, u_ip1, lambda_c_i):
  ...
  dyn, contact = EvaluateDynamics(planar_arm, context, s_halfway, u_halfway, lambda_c_i)
...
def AddCollocationConstraints(prog, planar_arm, context, N, x, u, lambda_c, timesteps):
  ...
  def CollocationConstraintHelper_1(vars):
      ...
      lambda_c_i = vars[2*(n_x+n_u): ]

      return CollocationConstraintEvaluator(planar_arm, context, timesteps[i+1] - timesteps[i], x_i, u_i, x_ip1, u_ip1, lambda_c_i,)[0]
  ...
  def CollocationConstraintHelper_2(vars):
    ...
    return CollocationConstraintEvaluator(planar_arm, context, timesteps[i+1] - timesteps[i], x_i, u_i, x_ip1, u_ip1, lambda_c_i)[1]
  ...
  for i in range(N - 1):
    ...
    prog.AddConstraint(CollocationConstraintHelper_1, lower_bound-eps, upper_bound+eps,np.hstack([x[i], x[i+1], u[i], u[i+1], lambda_c[i]]) )
    prog.AddConstraint(CollocationConstraintHelper_2, contact_lower_bound-eps, contact_upper_bound+eps,np.hstack([x[i], x[i+1], u[i], u[i+1], lambda_c[i]]) )
