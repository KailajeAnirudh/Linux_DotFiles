...
def get_grad(w, lambda_i, X=data_train, Y = target_train, w0 = 0):
    ...
    grad = -(1/X.shape[0])* sigmoid@(Y[:, np.newaxis]*X)+ lambda_i*w.T

    grad2 = np.zeros(X.shape[1])
    for i in range(X.shape[0]):
        sw = np.exp(-Y[i]*(w.T@X[i] + w0))
        grad2 += (Y[i]*X[i]*sw)/(1+sw)
    grad2 *= 1/X.shape[0]
    grad2 += (lambda_i)*(np.linalg.norm(w))
