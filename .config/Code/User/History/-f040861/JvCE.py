...
for i in range(gradient_loss.shape[0]):
    plt.text(50, np.log((i+1)**10), f'Slope for $\lambda = {regularization_lambdas[i]} : {slopes[i]:.4f}$')
plt.text(50, np.log((i+2)**10), f'Kappa for best fit: {-1/slopes[np.argmax(test_accuracy)]}')
...
ls = np.array([500, 1000, 2000, 4000, 9000, 12000])
...
for i, params in enumerate(product(batch_sizes, nesterov_momentum_factors, regularization_lambdas)):
    batch_size, rho, lambda_i = params
    wt = np.random.standard_normal((28*28))
    w0 = 0
    u = wt; u0 = w0
    for t in tqdm(range(niters)):
...
for i in range(len(batch_sizes)):
    fig = plt.figure()
    for j in range(len(regularization_lambdas)):
        plt.semilogy(np.arange(SGD_loss.shape[1]), SGD_loss[i*len(regularization_lambdas) + j])
    plt.ylabel('Training Loss (log-scale)')
    plt.xlabel('Weight Updates')
    plt.legend([f'$\lambda = {regularization_lambdas[k]}$, ' for k in range(len(regularization_lambdas))])
...
for i in range(len(batch_sizes)):
    for j in range(len(nesterov_momentum_factors)):
        ...
        for k in range(len(regularization_lambdas)):
            plt.semilogy(np.arange(SGD_nesterov_loss.shape[1]), SGD_nesterov_loss[i*len(batch_sizes)+j*len(nesterov_momentum_factors)+j])
        plt.ylabel('Training Loss (log-scale)')
        plt.xlabel('Weight Updates')
        plt.legend([f'$\lambda = {regularization_lambdas[k]}$, ' for k in range(len(regularization_lambdas))])
