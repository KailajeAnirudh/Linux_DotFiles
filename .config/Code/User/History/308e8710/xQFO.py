...
def dir_col(N, initial_state, jumpheight, tf, jumpheight_tol=5e-2):
  ...
  joint_limit_lower = np.array([-1e-1, 0.0, -1e-1, -3.14, 0, -3.14, 0])
  joint_limit_upper = np.array([1e-1, 1.0, 1e-1, 3.14, 3.14, 3.14, 3.14])
  ...
  prog.SetSolverOption(SolverType.kSnopt, "Major iterations limit", 300000) #30000
  # prog.SetSolverOption(SolverType.kSnopt, "Minor iterations limit", 300000) #30000
  prog.SetSolverOption(SolverType.kSnopt, "Minor feasibility tolerance", 5e-5)
  prog.SetSolverOption(SolverType.kSnopt, "Major optimality tolerance", 1e-6)
