...
def find_throwing_trajectory(N, initial_state, final_state, tf, jumpheight_tol=5e-2):
  ...
  u = np.array([prog.NewContinuousVariables(n_x, f"u_{i}") for i in  range(N)])
  lambda_c = np.array([prog.NewContinuousVariables(n_x, f"lambda_c_{i}") for i in  range(N-1)])
...
if __name__ == '__main__':
  ...
  tf = 3.0
  x_traj, u_traj, prog,  _, _ = find_throwing_trajectory(N, initial_state, final_state=final_state, tf=3, jumpheight_tol=5e-2)
  print("Done")
