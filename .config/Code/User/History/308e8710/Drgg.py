...
def find_throwing_trajectory(N, initial_state, final_state, tf, jumpheight_tol=5e-2):
  ...
  prog.AddBoundingBoxConstraint(final_state-np.ones_like(final_state)*1e-3, final_state+np.ones_like(final_state)*1e3, xf)
  ...
  for i in range(N-1):
       ...
       prog.AddQuadraticCost(0.5*500*(x[i][2]-final_state[2])**2)
