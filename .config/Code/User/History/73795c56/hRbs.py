...
def find_throwing_trajectory(N, initial_state, jumpheight, tf, jumpheight_tol=5e-2):
  ...
  file_name = "/home/anirudhkailaje/Documents/01_UPenn/02_MEAM5170/05_Madness/src/planar_walker.urdf"
  ...
  for i in range(N-1):
      ...
      #  prog.AddConstraint(x[i][1]<= xf[1])
      #  prog.AddBoundingBoxConstraint(1e-2, 2*np.pi-1e-2, x[i][2])
...
if __name__ == '__main__':
  ...
  q[0] = 0; q[1] = 0.8
  theta = -np.arccos(q[1])
  q[3] = theta; q[4] = -2 * theta/2
  q[5] = theta;   q[6] = -2 * theta/2
  initial_state[:7] = q
  final_state = initial_state
  # final_configuration = np.array([np.pi, 0])
  tf = 3.0
  x_traj, u_traj, prog,  _, _ = find_throwing_trajectory(N, initial_state, jumpheight=1.3, tf=1, jumpheight_tol=5e-2)
