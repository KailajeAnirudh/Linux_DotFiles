...
def complementary_filter_update(initial_rotation, angular_velocity, linear_acceleration, dt):
    ...
    R_1k = Rotation.from_rotvec(w_dt).as_matrix() @ initial_rotation.as_matrix()

    g_prime = (R_1k@linear_acceleration)/g
    g_prime /= np.linalg.norm(g_prime)
    ...
    w_acc = np.cross(g_prime, np.array([0,0,1]))
    theta = np.arccos(np.dot(g_prime, np.array([0,0,1])))
    quat = [np.sin(theta/2)*w_acc[0], np.sin(theta/2)*w_acc[1], np.sin(theta/2)*w_acc[2], np.cos(theta/2)]
    
    if g_prime[2] > 0:
        delta_q_acc = Rotation.from_quat([0, np.sqrt(0.5*(g_prime[2]+1)), g_prime[1]/np.sqrt(2*(g_prime[2]+1)), -g_prime[0]/np.sqrt(2*g_prime[2]+1)])
        q_I = Rotation.from_quat([0,0,0,1]) #Null quaternion

        get_alpha = lambda error_factor: min(1, max(0, 10*(error_factor-0.1)+1))
        alpha = get_alpha(abs(np.linalg.norm(linear_acceleration/g)-1))
        ...
        return Rotation.from_matrix(R_1k)
