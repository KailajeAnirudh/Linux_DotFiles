...
def complementary_filter_update(initial_rotation, angular_velocity, linear_acceleration, dt):
    ...
    R_1k = initial_rotation*Rotation.from_rotvec(w_dt) #Key mistake done, was premultiplying.
    ...
    alpha = get_alpha(abs(norm(linear_acceleration)/g-1)) #Using adaptive gain.

    delta_q_prime_acc = (1-alpha)*np.array([0,0,0,1]) + alpha*delta_q_acc.as_quat()
    delta_q_prime_acc /= np.linalg.norm(delta_q_prime_acc) #Blending
