...
def graph_search(world, resolution, margin, start, goal, astar):
    ...
    resolution_multiplier = original_map.map.sum()/np.ones_like(original_map.map).sum()

    print(f"Original resolution: {resolution}, Map Shape: {np.prod(original_map.map.shape)}, Occupancy: {resolution_multiplier}")
    if resolution_multiplier < 0.1:
        resolution_multiplier = 0.25
    threshold = 1.25
    
    
    if not np.any(np.array(original_map.map.shape) ==1) and np.prod(original_map.map.shape) > 10000:
