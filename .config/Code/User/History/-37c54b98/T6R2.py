...
def graph_search(world, resolution, margin, start, goal, astar):
    ...
    print(f"Original resolution: {resolution}, Map Shape: {np.prod(original_map.map.shape)}, Occupancy: {occupancy}")
    resolution_multiplier = 0.8
    threshold = 1.25
    
    res_lev = 0.35
    resolution = np.array([min(res_lev, original_resolution[0]), min(res_lev, original_resolution[1]), max(0.7, original_resolution[2])])
