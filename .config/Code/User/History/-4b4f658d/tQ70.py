...
# rise_time = timestamps_drone_y[np.argmin((0.8*0.5)-drone_y_positions)]
...
plt.hlines([steady_state_value, drone_y_positions.max()], [steady_state_time, max_time], [14, max_time+5], colors = 'black', label = 'Steady state')
...
plt.text(steady_state_time+0.1, steady_state_value+0.01, f'Steady State: {steady_state_value: .3f} m, Settling time: {steady_state_time} s')
# plt.text(rise_time+0.1, 0.2, f'Rise Time: {rise_time:.3f} s')
plt.xlabel('Time (s)')
plt.ylabel('Y Position (m)')
plt.title('Step Y Response')
plt.grid(True)
plt.legend()
plt.savefig('StepYResponse.jpg', dpi =300)
plt.show()
