...
initial_state = {'x': np.array([-(dist**2/2)**0.5, -(dist**2/2)**0.5, -0]),
                 'v': np.zeros(3,),
                 'q': np.array([ 0.1767767, 0.3061862, 0.3061862, 0.8838835 ]), # [i,j,k,w]
                 'w': np.zeros(3,)}
...
# (fig, axes) = plt.subplots(figsize = (12,10),nrows=2, ncols=1, sharex=True, num='Position vs Time')
# x = state['x']
# x_des = flat['x']
# ax = axes[0]
...
# # Add text in the lower left corner for np.diag() of the Kp, Kd, Kr, Kw of my_se3_control
# ax.text(0.05, 0.05, f"Kp: {np.diag(my_se3_control.Kp)}", transform=ax.transAxes)
# ax.text(0.05, 0.1, f"Kd: {np.diag(my_se3_control.Kd)}", transform=ax.transAxes)
# ax.text(0.05, 0.15, f"Kr: {np.diag(my_se3_control.Kr)}", transform=ax.transAxes)
# ax.text(0.05, 0.2, f"Kw: {np.diag(my_se3_control.Kw)}", transform=ax.transAxes)
...
# # os.mkdir('./plots/')
# plt.savefig(f'./plots/position_vs_time{time}.png')
