...
class dircol(LeafSystem):
    ...
    def __init__(self):
        ...
        self.com_des_input_port_index = self.DeclareVectorInputPort("com_des", 1).get_index()
        self.time_des_input_port_index = self.DeclareVectorInputPort("time_des", 1).get_index()


        # Output Ports #
        self.com_trajectory_output_port_index = self.DeclareVectorOutputPort("comtraj", lambda: AbstractValue.Make(BasicVector(4)),self.comtrajCB).get_index()
