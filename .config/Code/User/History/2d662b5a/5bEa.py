...
def estimate_rot(data_num=1):
    #load data
    imu = io.loadmat('/home/anirudhkailaje/Documents/01_UPenn/06_ESE6500/Robotics-Learning/02_UnscentedKalmanFilter/imu/imuRaw'+str(data_num)+'.mat')
    accel = imu['vals'][0:3,:]
    gyro = imu['vals'][3:6,:]
    T = np.shape(imu['ts'])[1]
    ...
    Q = np.diag([225/34, 225/34, 1815/34, 310/34, 270/34, 174/34]).astype(float)
    R =  np.diag([2300,2300,2300,1000,1000,800]).astype(float)/10
    # R = 5*np.diag([2.12152214, 2.58091551, 1.4783665 , 0.25833854, 0.31113115,0.27795984])
...
if __name__ == "__main__":
    ...
    num = 3
    vicon = io.loadmat("/home/anirudhkailaje/Documents/01_UPenn/06_ESE6500/Robotics-Learning/02_UnscentedKalmanFilter/vicon/viconRot" + str(num) + ".mat")
