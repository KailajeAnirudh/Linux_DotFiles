...
filenames = ['test_big.json', 'test_empty.json', 'test_forest.json', 'test_impossible.json', 'test_saw.json', 'test_window.json']
...
for filename in filenames:
    ...
    ax.text(start[0]-0.5, start[1]-0.5, start[2]-0.75, f'Solve Time: {end_time-start_time:.2f}s', color='green')
    if path is not None:
        ax.text(start[0]-0.5, start[1]-0.5, start[2]-1., f'Path length: {length:.2f}m', color='green')
plt.show()
print("Done")
