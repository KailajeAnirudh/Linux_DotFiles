...
for epoch in range(epochs): 
  ...
  for i, data in enumerate(testloader, 0):
    ...
    test_error_values.append(100-acc_te_t)
    loss = criterion(outputs, labels)
    average_test_loss += loss.item()
  average_test_accuracy /= len(testloader)
  average_test_loss /= len(testloader)
  test_error_values.append(average_test_accuracy)    
  test_loss_values.append(average_test_loss)
  print(f"Epoch: {(epoch+1):2d}/{epochs}, Step: {i+1}/{steps_per_epoch}, Test Loss: {average_test_loss:.3f}, Test Error: {average_test_accuracy:.3f}")
