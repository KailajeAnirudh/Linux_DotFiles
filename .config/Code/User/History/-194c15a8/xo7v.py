...
class PhaseSwitch(LeafSystem):
    def __init__(self, height, jump_time, x_traj):
        ...
        self.phase_port_index = self.DeclareAbstractOutputPort("Phase", lambda: AbstractValue.Make(BasicVector(1)), self.PhaseOutput).get_index()

    def DeterminePhase(self, output):
    ...
    def SetLandingOutput(self, output):
        phase = self.DeterminePhase()
        if phase == 2:
            output.set_value()
