...
class SE3Control(object):
    ...
    def __init__(self, quad_params):
        ...
        self.Kr = np.diag([2800, 2800, 150.0])*1.25
        self.Kw = np.diag([125, 125, 80.0])*1.25
...
def graph_search(world, resolution, margin, start, goal, astar):
    ...
    res_lev = resolution.max()*1.15 if map_size>32000 else 0.25
    ...
    while (not Planner.Path or np.linalg.norm(np.array(Planner.Path[-1]) - np.array(goal)) > error_threshold*np.linalg.norm(resolution)) and tries<=4:
        if tries == 2:
...
class WorldTraj(object):
    ...
    def __init__(self, world, start, goal):
        ...
        if self.path is not None:
            ...
            waypoint_distances = np.linspace(0, cumulative_distances[-1], int(cumulative_distances[-1]/(0.25*3.5)))
