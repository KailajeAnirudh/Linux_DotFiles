...
class OperationalSpaceWalkingController(LeafSystem):
    def __init__(self, gains: OscGains):
        ...
        #Dictionary for switching the costs and trajectory parameters for different states in the FSM
        self.tracking_costs = {"com_traj": self.gains.w_com,
        ...
        self.traj_input_ports = {"com_traj": self.DeclareAbstractInputPort("com_traj", AbstractValue.Make(trj)).get_index(),
    ...
    def get_traj_input_port(self, traj_name): return self.get_input_port(self.traj_input_ports[traj_name])
    ...
    def CalculateContactJacobian(self, fsm: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        J =  self.plant.CalcJacobianTranslationalVelocity(self.plant_context, JacobianWrtVariable.kV, pt_to_track.frame,
                                                        pt_to_track.pt, self.plant.world_frame(), self.plant.world_frame())

        JdotV = self.plant.CalcBiasTranslationalAcceleration(self.plant_context, JacobianWrtVariable.kV, pt_to_track.frame,
                                                            pt_to_track.pt, self.plant.world_frame(), self.plant.world_frame()).ravel()
    ...
    def SetupAndSolveQP(self,  context: Context) -> Tuple[np.ndarray, MathematicalProgram]:
        ...
        t = context.get_time(); fsm = get_fsm(t)
        ...
        for traj_name in self.trajs:
            ...
            y_ddot_i = JdotV_i + J_i@vdot
            # print(((yddot_cmd_i - y_ddot_i).T @ W_i @ (yddot_cmd_i - y_ddot_i)).shape)
            prog.AddQuadraticCost((yddot_cmd_i - y_ddot_i).T @ W_i @ (yddot_cmd_i - y_ddot_i))

            

        # TODO: Add Quadratic Cost on vdot using self.gains.w_vdot
        prog.AddQuadraticCost(self.gains.w_vdot*0.5*vdot.T@vdot)
        ...
        prog.AddLinearEqualityConstraint(J_c_dot_v+J_c@vdot, np.zeros_like(J_c_dot_v))

        # TODO: Add Friction Cone Constraint assuming mu = 1
        prog.AddLinearConstraint(np.array([[1, 0, -1],[-1, 0, -1]])@lambda_c, -np.inf*np.ones((2,1)), np.zeros((2,1)))
