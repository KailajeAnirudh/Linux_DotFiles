...
class OperationalSpaceWalkingController(LeafSystem):
    ...
    def SetupAndSolveQP(self,  context: Context) -> Tuple[np.ndarray, MathematicalProgram]:
        ...
        prog.AddLinearEqualityConstraint(M@vdot+Cv+G-B@u.reshape(-1,1)-J_c.T@lambda_c.reshape(-1,1), np.zeros_like(Cv))

        # TODO: Add Contact Constraint
        prog.AddLinearEqualityConstraint(J_c_dot_v+J_c@vdot, 0)

        # TODO: Add Friction Cone Constraint assuming mu = 1
        prog.AddLinearConstraint(np.array([[1, 0, -1],[-1, 0, -1]])@lambda_c <= np.array([[0, 0]]))
