...
class VAE(nn.Module):
    ...
    def decoder(self, latent_factor):
        decodelayer1 = torch.tanh(self.linear3(latent_factor))
        return torch.sigmoid(self.linear4(decodelayer1))

    def forward(self, x):
        mu, logsigma = self.encoder(x.view(-1, 196))
        latent_factor = self.reparameterization(mu, logsigma)
        return self.decoder(latent_factor), mu, logsigma
    
def get_loss(reconstructed_x, x, mu, logsigma):
    BCE_Loss = F.binary_cross_entropy(reconstructed_x, x.view(-1, 196), reduction='sum')
...
def validate(VariationalAE, dataloader):
    ...
    with torch.no_grad():
        for batch in (dataloader):
            ...
            BCE, KLD = get_loss(recon_batch, data, mu, logvar)
...
for epoch in range(epochs):
    ...
    for _, (data, _) in enumerate(dataloader):
        optimizer.zero_grad()
        recon_batch, mu, logvar = VariationalAE(data)
        BCE_loss, KL_Divergence = get_loss(recon_batch, data, mu, logvar)
...
sampled_images_from_latent_factors = VariationalAE.decoder(latent_factors)
sampled_images_from_latent_factors = sampled_images_from_latent_factors.detach().numpy()

fig, axs = plt.subplots(2, 4, figsize=(16, 8))
for i in range(8):
    axs[i//4, i%4].imshow(sampled_images_from_latent_factors[i].reshape(14, 14), cmap='gray')
    axs[i//4, i%4].axis('off')
