...
valdata = datasets.MNIST(root='./data', train=False, download=True, ValTransform=ValTransform)
...
for i in range(10):
    images = [img for img, label in traindata if label == i][:1000]
    ...
    for img in images:
        ...
        balanced_train_data.append(img)
...
dataloader = DataLoader(list(zip(balanced_train_data, new_dataset_labels)), batch_size=32, shuffle=True)
