...
def AddCollocationConstraints(prog, planar_arm, context, N, x, u, lambda_c, timesteps):
  ...
  for i in range(N - 1):
    def CollocationConstraintHelper_1(vars):
      ...
      return CollocationConstraintEvaluator(planar_arm, context, timesteps[i+1] - timesteps[i], x_i, u_i, x_ip1, u_ip1, lambda_c_i, lambda_c_ip1)[0]
    ...
    def CollocationConstraintHelper_2(vars):
      ...
      return CollocationConstraintEvaluator(planar_arm, context, timesteps[i+1] - timesteps[i], x_i, u_i, x_ip1, u_ip1, lambda_c_i, lambda_c_ip1)[1]
    ...
    prog.AddConstraint(CollocationConstraintHelper_1, lower_bound-eps, upper_bound+eps,np.hstack([x[i], x[i+1], u[i], u[i+1], lambda_c[i], lambda_c[i+1]]) )
    prog.AddConstraint(CollocationConstraintHelper_2(np.hstack([x[i], x[i+1], u[i], u[i+1], lambda_c[i], lambda_c[i+1]])), lower_bound-eps, upper_bound+eps)
