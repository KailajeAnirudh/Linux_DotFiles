...
class ResNet(nn.Module):
    def __init__( self, block: Type[Union[BasicBlock, Bottleneck, Transformer]], layers: List[int],
    ...
    def _make_layer( self, block: Type[Union[BasicBlock, Bottleneck, Transformer]],
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.avgpool(x); x = torch.flatten(x, 1); x = self.fc(x); x = self.fsigmoid(x)
        return x
