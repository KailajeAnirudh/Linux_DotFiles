...
def biped_col(N, initial_state, jumpheight, tf, jumpheight_tol=5e-2):
  '''
  Parameters:
  ...
  n_q = robot.num_positions();  n_v = robot.num_velocities()
  n_x = n_q + n_v;  n_u = robot.num_actuators()
  ...
  joint_limit_lower = np.array([-1, 0, -0.9, -1.39, -1.39, 0, 0]);  joint_limit_upper = np.array([1, 5, 0.6, 0.78, 0.78, 2.5, 2.5])
  ...
  prog.AddBoundingBoxConstraint(required_velocity-velocity_tol, required_velocity+velocity_tol, xf[n_q+1])
...
if __name__ == '__main__':
  ...
  tf = 3.0
  distance = 15.0
  x_traj, u_traj, prog, _, _ = biped_col(N, initial_state, jumpheight = 1.5, tf=3, jumpheight_tol=5e-2)